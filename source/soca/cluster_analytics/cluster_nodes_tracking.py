...
def es_index_new_item(body, es_index_name):
    doc_type = "item"
    add = es.index(index=es_index_name, doc_type=doc_type, body=body)
...
if __name__ == "__main__":
    ...
    try:
        command = subprocess.Popen(
            (commands["pbsnodes"] + commands["pbsnodes_args"]).split(), stdout=subprocess.PIPE, stderr=subprocess.STDOUT
        )
