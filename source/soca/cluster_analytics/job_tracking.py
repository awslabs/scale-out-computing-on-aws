...
def get_aws_pricing(ec2_instance_type):
    ...
    response = client.get_products(
        ...
        Filters=[
            {"Type": "TERM_MATCH", "Field": "usageType", "Value": "BoxUsage:" + ec2_instance_type},
        ],
    )
...
def es_index_new_item(body, index):
    doc_type = "item"
    add = es.index(index=index, doc_type=doc_type, body=body)
                    ...
                    if data.__len__() != 4:
                        ...
                        if job_id in output.keys():
                            output[job_id].append(
                                {"utc_date": timestamp, "job_state": job_state, "job_id": job_id, "job_data": job_data}
                            )
                        else:
                            output[job_id] = [
                                {"utc_date": timestamp, "job_state": job_state, "job_id": job_id, "job_data": job_data}
                            ...
                            if used_resources:
                                ...
                                for res in used_resources:
                                    ...
                                    if resource_name == "select":
                                        ...
                                        if mpiprocs:
                                            job_info["mpiprocs"] = int(
                                                re.search(r"mpiprocs=(\d+)", resource_value).group(1)
                                            )
                                    ...
                                    job_info[resource_name] = (
                                        int(resource_value) if resource_value.isdigit() is True else resource_value
                                    )
                                ...
                                job_info["job_uuid"] = base64.b64encode(
                                    f"{job_id}_{cluster_id}_{job_info['end_iso']}".encode("utf-8")
                                ).decode("utf-8")
                                ...
                                if "fsx_lustre_bucket" in job_info.keys():
                                    if job_info["fsx_lustre_bucket"] != "false":
                                        if "fsx_lustre_size" in job_info.keys():
                                            ...
                                            # default lustre size
                                            job_info["estimated_price_fsx_lustre"] = (
                                                1200 * fsx_lustre * (simulation_time_seconds_with_penalty / 3600)
                                            )

                                if job_info["instance_type"] not in pricing_table.keys():
                                    pricing_table[job_info["instance_type"]] = get_aws_pricing(
                                        job_info["instance_type"]
                                    )

                                job_info["estimated_price_ec2_ondemand"] = (
                                    simulation_time_seconds_with_penalty
                                    * (pricing_table[job_info["instance_type"]]["ondemand"] / 3600)
                                    * job_info["nodect"]
                                )
