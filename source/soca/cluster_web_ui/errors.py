...
def all_errors(error_name, trace=False):
    error_list = {
        ...
        "UNABLE_TO_ADD_USER_TO_GROUP": [f"User/Group created but could not add user to his group {trace if trace is not False else ''}", 230],
        ...
        "MISSING_DS_RESET_LAMBDA": [f"Flask could not locate SOCA_DS_RESET_PW_LAMBDA variable. Make sure you started the UI with socawebui.sh", 233],
        "DS_PASSWORD_COMPLEXITY_ERROR": [f"Your password does not meet complexity requirements. Use one uppercase, one lowercase, one digit and/or one special char and cannot contains your username. 8 chars min",234],
        ...
        "DS_PASSWORD_USERNAME_IS_ADMIN":  [f"Admin is an account restricted to ActiveDirectory Main User. Please pick a different name", 237],
