...
class Config(object):
    ...
    DEFAULT_CACHE_TIME = (
        120  # 2 minutes. Change this value to optimize performance in case you have a large number of concurrent user
    )
    ...
    MAX_SIZE_ONLINE_PREVIEW = (
        150000000  # in bytes (150mb by default), maximum size of file that can be visualized via the web editor
    )
    ...
    # COGNITO
    ENABLE_SSO = False
    COGNITO_OAUTH_AUTHORIZE_ENDPOINT = (
        "https://<YOUR_COGNITO_DOMAIN_NAME>.auth.<YOUR_REGION>.amazoncognito.com/oauth2/authorize"
    )
    COGNITO_OAUTH_TOKEN_ENDPOINT = (
        "https://<YOUR_COGNITO_DOMAIN_NAME>.auth.<YOUR_REGION>.amazoncognito.com/oauth2/token"
    )
    COGNITO_JWS_KEYS_ENDPOINT = (
        "https://cognito-idp.<YOUR_REGION>.amazonaws.com/<YOUR_REGION>_<YOUR_ID>/.well-known/jwks.json"
    )
    ...
    DCV_WINDOWS_AUTOLOGON = (
        True  # enable or disable autologon. If disabled user will have to manually input Windows password
    )
