    ...
    def put(self, session_number):
        ...
        if check_session:
            ...
            if check_session.session_state != "stopped":
                return errors.all_errors(
                    "DCV_RESTART_ERROR", f"This DCV desktop is not stopped. You can only restart a stopped desktop."
                )
            try:
                ...
                if e.response["Error"].get("Code") == "DryRunOperation":
                    ...
                    return errors.all_errors(
                        "DCV_RESTART_ERROR", f"Unable to restart instance {instance_id} due to {e}"
                    )
        else:
            return errors.all_errors(
