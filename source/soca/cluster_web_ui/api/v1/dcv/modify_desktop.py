...
class ModifyDesktop(Resource):
    ...
    def put(self, session_number):
        ...
        if args["os"] is None or args["session_number"] is None or args["instance_type"] is None:
            return errors.all_errors('CLIENT_MISSING_PARAMETER', "os (str), session_number (str), instance_type (str)  are required.")
        ...
        if args["instance_type"].lower() not in all_instances:
            return errors.all_errors('DCV_MODIFY_ERROR', f"{args['instance_type'].lower()} is not authorized by your Admin.")
        ...
        if check_session:
            ...
            if check_session.session_state != "stopped":
                return errors.all_errors('DCV_RESTART_ERROR', f"This DCV desktop is not stopped. You can only modify a stopped desktop.")
            try:
                client_ec2.modify_instance_attribute(InstanceId=instance_id, InstanceType={'Value': args["instance_type"].lower()}, DryRun=True)
            except ClientError as e:
                if e.response['Error'].get('Code') == 'DryRunOperation':
                    try:
                        client_ec2.modify_instance_attribute(InstanceId=instance_id,InstanceType={'Value': args["instance_type"].lower()})
                        check_session.session_instance_type = args["instance_type"].lower()
                        db.session.commit()
                        return {"success": True, "message": f"Your DCV desktop has been updated successfully to {args['instance_type'].lower()}"}, 200
                    except ClientError as err:
                        if "not supported for instances with hibernation configured." in err.response['Error'].get('Code'):
                            return errors.all_errors('DCV_MODIFY_ERROR', f"Your instance has been started with hibernation enabled. You cannot change the instance type. Start a new session with Hibernation disabled if you want to be able to change your instance type. ")
                        else:
                            return errors.all_errors('DCV_MODIFY_ERROR', f"Unable to modify EC2 instance {instance_id} type due to {err}")
                else:
                    return errors.all_errors('DCV_MODIFY_ERROR', f"Unable to modify EC2 instance {instance_id}  due to {e}")

        else:
            return errors.all_errors('DCV_MODIFY_ERROR', f"Unable to retrieve this session. It's possible your session has been deleted.")