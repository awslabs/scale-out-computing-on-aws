                ...
                if check_session.session_state == "stopped":
                    ...
                    try:
                        ...
                        if e.response["Error"].get("Code") == "DryRunOperation":
                            ...
                            return errors.all_errors(
                                "DCV_STOP_ERROR", f"Unable to hibernate instance ({instance_id}) due to {e}"
                            )
                ...
                # Stop Instance
                    ...
                    try:
                        ...
                        if e.response["Error"].get("Code") == "DryRunOperation":
                            ...
                            return errors.all_errors(
                                "DCV_STOP_ERROR", f"Unable to stop instance ({instance_id}) due to {e}"
                            )

            else:
                # Terminate instance
                stack_name = str(
                    read_secretmanager.get_soca_configuration()["ClusterId"] + "-" + session_name + "-" + user
                )
                try:
