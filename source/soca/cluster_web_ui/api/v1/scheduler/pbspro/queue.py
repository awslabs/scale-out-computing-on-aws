...
class Queue(Resource):
    ...
    def post(self):
        ...
        if queue_name is None:
            return {
                "success": False,
                "message": "name (str) is required parameter",
            }, 400

        if queue_type not in QUEUE_TYPE:
            return {
                "success": False,
                "message": "Invalid queue type, must be alwayson or ondemand",
            }, 400
            ...
            if queue_type == "ondemand":
                for command in commands_ondemand:
                    try:
                        subprocess.Popen(
                            shlex.split(
                                config.Config.PBS_QMGR + ' -c "' + command + '"'
                            )
                        )
                    except Exception as err:
                        return {
                            "success": False,
                            "message": "Error with " + command + " Trace: " + str(err),
                        }, 500
            else:
    ...
    def delete(self):
        ...
        if queue_name not in all_queues:
            return {
                "success": False,
                "message": "Queue does not exist. Create it first",
            }

        try:
            delete_queue = subprocess.Popen(
                shlex.split(
                    config.Config.PBS_QMGR + ' -c "delete queue ' + queue_name + '"'
                )
            )
            ...
            return {
                "success": False,
                "message": "Unable to delete queue: "
                + str(err)
                + ". Trace: "
                + str(delete_queue),
            }, 200
