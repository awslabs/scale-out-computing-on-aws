from flask import session, Blueprint, redirect, request, Response, flash
...
def dcv_create_session():
    try:
        ...
        if user_sessions.__len__() >= parameters.authorized_dcv_session_count():
            ...
            launch_job = dcv.build_qsub(username, session_number, walltime, instance_type)
            if launch_job is True:
                return redirect('/remotedesktop')
            else:
                flash('Error while trying to launch your session: Please verify you can submit job to the desktop queue. Error:<hr> ' +str(launch_job))
                return redirect('/remotedesktop')

    except Exception as e:
        flash('Error while trying to launch your session: Please verify you can submit job to the desktop queue (eg: verify budget restrictions for this queue, queue syntax). Trace: ' + str(e))
        return redirect('/remotedesktop')
