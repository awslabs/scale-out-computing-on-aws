...
def sso_authorization(code):
    authorization = (
        "Basic "
        + base64.b64encode((config.Config.COGNITO_APP_ID + ":" + config.Config.COGNITO_APP_SECRET).encode()).decode()
    )
    ...
    if claims:
        try:
            ...
            return {"success": False, "message": "Error reading SSO claims. " + str(claims) + " Err: " + str(err)}
