...
from models import AmiList, LinuxDCVSessions, db
...
def get_ami_info(image=False):
    ...
    if image is False:
        for session_info in AmiList.query.filter(AmiList.is_active == True).filter(AmiList.ami_type != "windows").all():
            ami_info[session_info.ami_label] = {"ami_id": session_info.ami_id, "ami_base_os": session_info.ami_type}
    else:
        check_image = (
        ...
        if check_image:
            ami_info[check_image.ami_label] = {"ami_id": check_image.ami_id, "ami_base_os": check_image.ami_type}
    return ami_info
...
"""
        ...
        return Response(
            ...
            mimetype="text/txt",
            headers={
                "Content-disposition": "attachment; filename=" + session["user"] + "_soca_" + str(dcv_session) + ".dcv"
            },
        )
