...
def index():
    ...
    return render_template(
        "admin/ami_management.html", user=session["user"], ami_infos=ami_infos, region_name=get_region()
    )
...
def ami_create():
    ...
    list_images = post(
        ...
        headers={"X-SOCA-USER": session["user"], "X-SOCA-TOKEN": session["api_key"]},
        data={"os": choose_os, "ami_label": ami_label, "root_size": root_size, "ami_id": ami_id},
        verify=False,
    )  # nosec
    if list_images.status_code == 200:
