...
def create():
    ...
    create_desktop = post(f"{config.Config.FLASK_ENDPOINT}/api/dcv/desktop/{request.form['session_number']}/windows",
                          headers={"X-SOCA-USER": session["user"], "X-SOCA-TOKEN": session["api_key"]},
...
@login_required
def modify():
    session_number = request.args.get("session_number", None)
    instance_type = request.args.get("instance_type", None)
...
def schedule():
    ...
    logger.info(f"Received following parameters {request.args} to modify DCV Windows schedule")
