...
def retrieve_target_node(cluster_id: str) -> dict:
    node_information = {}
    ...
    for page in ec2_iterator:
        for reservation in page["Reservations"]:
            for instance in reservation["Instances"]:
                node_information[instance["InstanceId"]] = {}
                node_information[instance["InstanceId"]]["soca_cluster_id"] = cluster_id
                node_information[instance["InstanceId"]][
                    "target_node_uuid"
                ...
                for tag in instance["Tags"]:
                    node_information[instance["InstanceId"]][tag["Key"]] = tag["Value"]

                # Add all desktop info to Analytics
                # https://boto3.amazonaws.com/v1/documentation/api/latest/reference/services/ec2.html#EC2.Client.describe_instances
                for k, v in instance.items():
                    if k != "Tags":
                        node_information[instance["InstanceId"]][k] = v
...
if __name__ == "__main__":
    ...
    _index_name = f"edh_target_nodes_{_cluster_id}"

    _log_file_location = f"/opt/edh/{_cluster_id}/cluster_manager/analytics/logs/target_nodes_tracking.log"
    logger = SocaLogger(name="analytics_desktop_hosts_tracking").rotating_file_handler(
        file_path=_log_file_location
    )

    logger.info(f"Tracking active SOCA Target Nodes . Log: {_log_file_location}")
    ...
    _current_nodes = retrieve_target_node(cluster_id=_cluster_id)

    if len(_current_nodes) > 0:
        for node_data in _current_nodes.values():
            _index_data = _analytics_client.index(index=_index_name, body=node_data)
            if _index_data.success:
                logger.info(f"RECORD INDEXED SUCCESSFULLY > {node_data}")
            else:
                logger.error(f"Error while indexing {node_data}")
    else:
        logger.info("No active SOCA Target Nodes found")
