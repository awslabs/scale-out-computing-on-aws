...
case "$1" in
    ...
    if [[ -z $status_check_process ]]; then
        ...
        # Launching process
        $UWSGI_BIN --master --https $UWSGI_BIND,cert.crt,cert.key --wsgi-file $UWSGI_FILE --processes $UWSGI_PROCESSES --log-maxsize 104857600 --threads $UWSGI_THREADS --daemonize logs/uwsgi.log --enable-threads --buffer-size $BUFFER_SIZE --check-static /opt/edh/${EDH_CLUSTER_ID}/cluster_manager/web_interface/static  --check-static /opt/edh/${EDH_CLUSTER_ID}/cluster_manager/web_interface/templates ${UWSGI_OPTIONS}
