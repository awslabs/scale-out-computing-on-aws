...
def index():
    ...
    all_instances = utils_remote_desktop_common.generate_allowed_instances_list(
        baseos="windows", architecture="x86_64"
    )
    ...
    return render_template(
        ...
        auth_provider=config.Config.DIRECTORY_AUTH_PROVIDER,
        netbios="false"
        if config.Config.DIRECTORY_AUTH_PROVIDER in ["openldap", "existing_openldap"]
        else config.Config.DIRECTORY_NETBIOS,
...
def generate_client():
    ...
    if check_session:
        if config.Config.DIRECTORY_AUTH_PROVIDER in ["aws_ds_managed_activedirectory", "aws_ds_simple_activedirectory"]:
            user = f"{config.Config.DIRECTORY_NETBIOS}\{session['user']}"
            session_file = f"""
[version]
format=1.0

[connect]
host={SocaConfig(key="/configuration/LoadBalancerDNSName").get_value().get("message")}
port=443
sessionid=console
user={user}
