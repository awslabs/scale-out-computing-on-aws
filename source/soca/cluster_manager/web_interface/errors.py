...
def all_errors(error_name: str, trace=False):
    error_list = {
    ...
    if error_name in error_list.keys():
        logger.error(
            f"Error Code {error_name}: {error_list[error_name][0]}, return code {error_list[error_name][1]}"
        )
        return {"success": False, "message": error_list[error_name][0]}, error_list[
            error_name
        ][1]
    else:
        logger.error(f"Unknown error caused by: {trace}")
        return {
            "success": False,
            "message": f"Unknown error caused by {trace}",
        }, 500
