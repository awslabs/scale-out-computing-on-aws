    ...
    for _page in _ec2_iterator:
        for _reservation in _page.get("Reservations", {}):
            for _instance in _reservation.get("Instances", {}):
                _private_dns = _instance.get("PrivateDnsName", "unknown.unknown").split(
                    "."
                )[0]
                if _private_dns:
                    ...
                    print(
                        f"Error: private_dns is empty for instance {_instance['InstanceId']}"
                    )
...
def register_instance_to_target_group(target_group_arn: str, instance_id: str) -> bool:
    ...
    if register_ec2["ResponseMetadata"]["HTTPStatusCode"] == 200:
        ...
        print(
            f"Error: EC2 instance {instance_id} not registered to TG {target_group_arn}"
        )
        print(register_ec2)
        return False
...
def create_new_target_group(
    instance_dns: str, vpc_id: str, instance_id: str, cluster_id: str
):
