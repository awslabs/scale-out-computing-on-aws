...
def list(
    ...
    if scheduler_identifier:
        _filters.append(
            {"Name": "tag:soca:SchedulerIdentifier", "Values": [scheduler_identifier]}
        )

    if stack_id:
        _filters.append({"Name": "tag:soca:StackId", "Values": [stack_id]})

    if "ondemand" in node_lifecycles:
        _filters.append(
            {"Name": "tag:soca:KeepForever", "Values": ["false", "False", "FALSE"]},
        )
...
def create_alwayson(
    ...
    _job.job_state = "QUEUED"  # alwayson will always use QUEUED. This is just to valide the SocaHpcJob model, and will not be needed as SOCA will now manage node lifecycle
...
def list_alwayson(alwayson_identifier):
    filters = [
        {"Name": "instance-state-name", "Values": ["pending", "running"]},
        {"Name": "tag:soca:ClusterId", "Values": [get_cluster_id()]},
        {"Name": "tag:soca:KeepForever", "Values": ["True", "true", "TRUE"]},
        {"Name": "tag:soca:JobName", "Values": [alwayson_identifier]},
    ]
