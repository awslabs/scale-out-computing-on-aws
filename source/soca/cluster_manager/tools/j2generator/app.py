...
@click.option(
    ...
    "get_template",
    required=True,
    help="Path to your Jinja2 Template file based on the value of --template-dirs",
)
@click.option(
    "--template-dirs",
    "template_dirs",
    required=True,
    multiple=True,
    help="Path to include any additional jinja2 files",
)
@click.option(
    "--add-value",
    "add_value",
    multiple=True,
    help='Add Additional keys/value to the list of variables e.g: --add-value "KEY=/job/NodeType VALUE=login_node TYPE=str"',
)
@click.option(
    "--output-file",
    "output_file",
    help="Location of the output file, support S3 URI and local file",
)
...
def main(
    ...
    _get_variables = SocaConfig(key=ssm_key, cache_admin=False).get_value(
        cache_result=False
    )
    ...
    if add_value is not None:
        ...
        for entry in list(add_value):
            ...
            if _key_name in _variables.keys():
                click.echo(
                    f"Error: {_key_name} seems to already exist in the variable list"
                )
                sys.exit(1)
            else:
                _allowed_types = {
                ...
                if _type_func is False:
                    click.echo(
                        f"Error: Invalid type {_value_type}. Supported types: {', '.join(_allowed_types.keys())}"
                    )
                    sys.exit(1)
                else:
    ...
    if output_file:
        ...
        # Regex pattern for s3:// URI format
        _s3_pattern = r"^s3://([^/]+)/?(.*)$"
        _check_if_s3_uri = re.match(_s3_pattern, output_file)
        if _check_if_s3_uri:
            logger.debug(f"Saving file to S3 URI: {output_file}")
            bucket = _check_if_s3_uri.group(1)
            key = _check_if_s3_uri.group(2)
            _req = SocaJinja2Generator(
                get_template=get_template,
                template_dirs=template_dirs,
                variables=_variables,
            ).to_s3(bucket_name=bucket, key=key, autocast_values=True)
        else:
            logger.debug(f"Saving local file: {output_file}")
            _req = SocaJinja2Generator(
                get_template=get_template,
                template_dirs=template_dirs,
                variables=_variables,
            ).to_file(output_file_path=output_file, autocast_values=True)

    else:
        logger.debug(f"Rendering j2 template {get_template} to stdout")
        _req = SocaJinja2Generator(
            get_template=get_template, template_dirs=template_dirs, variables=_variables
