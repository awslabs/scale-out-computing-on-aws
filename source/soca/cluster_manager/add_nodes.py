...
def verify_ri_saving_availabilities(instance_type, instance_type_info):
    if instance_type not in instance_type_info.keys():
        instance_type_info[instance_type] = {"current_instance_in_use": 0, "current_ri_purchased": 0}
        token = True
        next_token = ""
        ...
        get_ri_count = ec2.describe_reserved_instances(
            Filters=[{"Name": "instance-type", "Values": [instance_type]}, {"Name": "state", "Values": ["active"]}]
        )
...
def verify_vcpus_limit(instance_type, desired_capacity, quota_info):
    ...
    if not quota_info or instance_type not in quota_info.keys():
        ...
        while token is True:
            if next_token is False:
                response = servicequotas.list_service_quotas(ServiceCode="ec2", MaxResults=100)

            else:
                response = servicequotas.list_service_quotas(ServiceCode="ec2", MaxResults=100, NextToken=next_token)
            try:
                next_token = response["NextToken"]
            except KeyError:
                token = False
            ...
            for quota in response["Quotas"]:
                ...
                if "running on-demand" in quota["QuotaName"].lower() and instance_family in quota["QuotaName"].replace(
                    "Running", ""
                ):
    ...
    if "standard" in quota_name.lower():
        instances_family_allowed_in_quota = (
            re.search(r"running on-demand standard \((.*)\) instances", quota_name.lower()).group(1).split(",")
        )
    else:
    ...
    if max_vcpus_allowed >= (running_vcpus + total_vcpus_requested):
        ...
        return {
            "message": "Job cannot start due to AWS Service limit. Max Vcpus allowed {}. Detected running Vcpus {}. Requested Vcpus for this job {}. Quota Name {}".format(
                max_vcpus_allowed, running_vcpus, total_vcpus_requested, quota_name
            ),
...
def check_config(**kwargs):
        ...
        for item in kwargs["weighted_capacity"].split("+"):
            try:
                ...
                error = return_message(
                    "All values specified for --weighted_capacity must be integers. Found: " + str(item)
                )
    ...
    if kwargs["force_ri"] is True and kwargs["spot_price"] is False:
        ...
        for instance_type in kwargs["instance_type"]:
            ...
            if (check_ri[instance_type]["current_instance_in_use"] + int(kwargs["desired_capacity"])) > check_ri[
                instance_type
                ...
                instance_type_info[instance_type] = {
                    "current_instance_in_use": check_ri[instance_type]["current_instance_in_use"]
    ...
    if kwargs["fsx_lustre"] is not False:
        ...
        if kwargs["fsx_lustre_size"] is not False:
            ...
            if int(kwargs["fsx_lustre_size"]) not in fsx_lustre_capacity_allowed:
                error = return_message(
                    "fsx_lustre_size must be: " + ",".join(str(x) for x in fsx_lustre_capacity_allowed)
                )
            else:
    ...
    SpotFleet = (
        ...
        if (
            ...
            and (int(kwargs["desired_capacity"]) > 1 or kwargs["instance_type"].__len__() > 1)
        )
        else False
    )
        ...
        if isinstance(kwargs["subnet_id"], int):
            ...
            for subnet in kwargs["subnet_id"]:
                if subnet not in soca_configuration["PrivateSubnets"]:
                    error = return_message(
                        "Incorrect subnet_id. Must be one of " + ",".join(soca_configuration["PrivateSubnets"])
                    )
    ...
    if kwargs["instance_profile"]:
        try:
            kwargs["instance_profile"] = iam.get_instance_profile(InstanceProfileName=kwargs["instance_profile"])[
                "InstanceProfile"
            ]["Arn"]
        except Exception as err:
            error = return_message(
    ...
    mapping = {
        ...
        "diversified": {"ASG": "capacity-optimized", "SpotFleet": "diversified", "accepted_values": ["diversified"]},
        "capacity-optimized": {
    ...
    if kwargs["spot_allocation_count"] is not False:
        if isinstance(kwargs["spot_allocation_count"], int):
            ...
            error = return_message(
                "spot_allocation_count (" + str(kwargs["spot_allocation_count"]) + ") must be an integer"
            )
...
def main(**kwargs):
    try:
        ...
        if params["keep_forever"] is True:
            cfn_stack_name = (
                soca_configuration["ClusterId"] + "-keepforever-" + params["queue"] + "-" + params["stack_uuid"]
            )
        ...
        parameters_list = {
            ...
            "FSxLustreConfiguration": {"Key": "fsx_lustre_configuration", "Default": False},
            "ImageId": {"Key": "instance_ami", "Default": soca_configuration["CustomAMI"]},
            ...
            "S3InstallFolder": {"Key": None, "Default": soca_configuration["S3InstallFolder"]},
            "SchedulerPrivateDnsName": {"Key": None, "Default": soca_configuration["SchedulerPrivateDnsName"]},
            "ScratchSize": {"Key": "scratch_size", "Default": 0},
            "AdditionalSecurityGroupIds": {"Key": "security_groups", "Default": None},
            "SecurityGroupId": {"Key": None, "Default": soca_configuration["ComputeNodeSecurityGroup"]},
            "SchedulerHostname": {"Key": None, "Default": soca_configuration["SchedulerPrivateDnsName"]},
            "SolutionMetricsLambda": {"Key": None, "Default": soca_configuration["SolutionMetricsLambda"]},
            "SpotAllocationCount": {"Key": "spot_allocation_count", "Default": False},
            "SpotAllocationStrategy": {"Key": "spot_allocation_strategy", "Default": "capacity-optimized"},
            "SpotFleetIAMRoleArn": {"Key": None, "Default": soca_configuration["SpotFleetIAMRoleArn"]},
        ...
        if can_launch is True:
            try:
                cloudformation.create_stack(
                    StackName=cfn_stack_name, TemplateBody=cfn_stack_body["output"], Tags=cfn_stack_tags
                )

                return {"success": True, "stack_name": cfn_stack_name, "compute_node": "job" + str(params["job_id"])}
        ...
        fname = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
        return return_message(
            str(e) + ": error:" + str(exc_type) + " " + str(fname) + " " + str(exc_tb.tb_lineno) + " " + str(kwargs)
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--job_name", nargs="?", required=True, help="Job Name for which the capacity is being provisioned"
    )
    parser.add_argument(
        "--job_owner", nargs="?", required=True, help="Job Owner for which the capacity is being provisioned"
    )
    ...
    # Optional
    parser.add_argument(
        "--force_ri", default=False, help="If True, job can only run if we have reserved instance available"
    )
    parser.add_argument("--base_os", default=False, help="Specify custom Base OK")
    parser.add_argument(
        "--terminate_when_idle", default=0, nargs="?", help="If instances will be terminated when idle for N minutes"
    )
    ...
    parser.add_argument(
        "--fsx_lustre_per_unit_throughput", default=200, help="Storage baseline if FSX type is Persistent"
    )
    ...
    parser.add_argument(
        "--job_project", nargs="?", default=False, help="Job Owner for which the capacity is being provisioned"
    )
    ...
    parser.add_argument(
        "--spot_allocation_count", default=False, nargs="?", help="When using mixed OD and SPOT, choose %% of SPOT"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--security_groups", default=False, help="Configure additional security groups for your compute nodes"
    )
