            ...
            if scaling_mode == "multiple_jobs":
                ...
                for resource in response['StackResources']:
                    if resource['ResourceType'] == 'AWS::EC2::SpotFleet':
                        ...
                        response=ec2.describe_spot_fleet_request_history(SpotFleetRequestId=spotfleet, EventType='fleetRequestChange', StartTime=now_minus_ten_minutes, MaxResults=1)
                        ...
                        if now > (spot_fleet_last_activity + datetime.timedelta(minutes=10)):
                            logpush(job_id + ' Spotfleet last activity was more than 10 mins ago but job has not started yet, rollback compute_node value')
                    ...
                    if resource['ResourceType'] == 'AWS::AutoScaling::AutoScalingGroup':
                        ...
                        if now > (asg_last_activity_end_time + datetime.timedelta(minutes=10)):
                            logpush(job_id + ' asg last activity was more than 10 minutes ago but job has not started yet, rollback compute_node value')
                            new_job_select = job_select_resource.split(':compute_node')[0] + ':compute_node=tbd'
                            qalter_cmd = [system_cmds['qalter'], "-l", "stack_id=", "-l", "select=" + new_job_select, str(job_id)]
                            run_command(qalter_cmd, "call")
                        else:
                ...
                if now > (stack_creation_time + datetime.timedelta(minutes=30)):
                    ...
                    logpush(job_id + " Stack has been created for less than 30 minutes. Let's wait a bit before killing the CI and resetting the compute_node value")
                    return True

        elif check_stack_status['Stacks'][0]['StackStatus'] == 'CREATE_IN_PROGRESS':
            logpush(job_id + ' is queued but has a valid CI assigned. However CloudFormation stack is not completed yet so we exit the script.')
            return True

        elif check_stack_status['Stacks'][0]['StackStatus'] in ['CREATE_FAILED', 'ROLLBACK_COMPLETE', 'ROLLBACK_FAILED']:
            logpush(job_id + ' is queued but has a valid CI assigned. However CloudFormation stack is ' + str(check_stack_status['Stacks'][0]['StackStatus']) +'.  Because job has not started by then, rollback compute_node value and delete stack')
            new_job_select = job_select_resource.split(':compute_node')[0] + ':compute_node=tbd'
...
def available_capacity(job_hash):
    ...
    pbsnodes_output = {}
    try:
        pbsnodes_output = json.loads((run_command((system_cmds['pbsnodes'] + pbsnodes_args).split(), "check_output")).decode('utf-8'))
        if 'nodes' in pbsnodes_output.keys():
            for hostname, host_data in pbsnodes_output['nodes'].items():
                if (host_data['state'] == 'free') and (host_data['resources_available']['compute_node'] == 'job' + job_hash) \
                        and (datetime.datetime.now().timestamp() > host_data['last_state_change_time'] + 60):
                    if 'ncpus' in host_data['resources_assigned'].keys():
                        free_cpus += int(host_data['resources_available']['ncpus']) - int(host_data['resources_assigned']['ncpus'])
                    else:
        ...
        if len(pbsnodes_output) == 0: ## pbsnodes_output could return an empty string when there are no nodes in the cluster
            pass
        else:
    ...
    for queue_name in queues:
        ...
        if scaling_mode == "multiple_jobs":
            ...
            for job_hash, job_data in get_jobs.items():
                ...
                queued_jobs_being_provisioned_in_hash = [job_data[k] for k, v in job_data.items() if v['get_job_state'] == 'Q' and "compute_node" in v['get_job_resource_list']["select"]]
            ...
            if skip_queue is False:
                ...
                for job_hash, hash_data in get_jobs.items():
                    ...
                    if 'terminate_when_idle' not in job_parameter_values.keys():
                        logpush('scaling_mode is set to multiple_jobs but terminate_when_idle is not specified in the queue_mapping.yml or at job submission. Exiting ...')
                        exit(1)
                    ...
                    for job_id in jobs_in_hash:
                        ...
                        if job_data['get_job_state'] == 'Q':
                            ...
                            if check_compute_unit:
                                ...
                                try:
                                    if capacity_being_provisioned(job_data['get_job_resource_list']['stack_id'], job_id, job_data['get_job_resource_list']['select'], scaling_mode) is True:
                            ...
                            for res in job_required_resource:
                                if res == "ncpus":
                                    if int(job_required_resource[res]) > int(min(weighted_capacity)):
                                        run_command([system_cmds['qalter'], "-l", "error_message=Job_requires_ncpus_"+str(job_required_resource[res])+ "_but_minimum_weighted_capacity_for_corresponding_instances_is_" + str(min(weighted_capacity)) + "_Please_delete_the_job_and_resubmit_with_updated_resource_requirements", str(job_id)], "call")
                                        logpush('Error: Job ' + str(job_id) + ' requires ' + str(job_required_resource[res]) + ' ncpus but minimum weighted_capacity for corresponding instances is: '+ str(min(weighted_capacity)) + '. Job may not run!. Please delete the job and resubmit with updated resource requirements')
                                ...
                                if res == "mem":
                                    ...
                                    if "kb" in job_required_resource[res].lower():
                                        ...
                                        job_required_mem = int(job_required_resource[res].lower().strip("b"))/1024/1024
                                    if job_required_mem > int(min(memory_required_instances)):
                                        run_command([system_cmds['qalter'], "-l", "error_message=Job_requires_" + str(job_required_mem) + "_MiB_mem_but_minimum_available_memory_for_corresponding_instances_is_" + str(min(memory_required_instances)) + "_MiB_Please_delete_the_job_and_resubmit_with_updated_resource_requirements", str(job_id)], "call")
                                        logpush('Error: Job ' + str(job_id) + ' requires ' + str(job_required_mem) + ' MiB mem. Specified instance types for this job are: ' + str(instance_types))
                                        logpush('.....: Available memory for corresponding instances is: '+ str(sorted(memory_required_instances)) + ' MiB. Job may not run as it could start on the smallest instance!')
                                        logpush('.....: Please delete the job and update required resources accordingly')
                                        logpush('Skipping job: ' + str(job_id))
                                        skip_job = True
                    ...
                    logpush("Completed full iteration for job_hash: " + str(job_hash) + ". Total required capacity is: " + str(hash_cpu_ct))
                    ...
                    if hash_cpu_ct > 0:
                        ...
                        try:
                            ...
                            exc_type, exc_obj, exc_tb = sys.exc_info()
                            fname = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
                            logpush("max_running_jobs: Error occurred when trying to determine if job can start: {}, {}, {}".format(exc_type, fname, exc_tb.tb_lineno))
                            sys.exit(1)
                        ...
                        if hash_cpu_ct > 0:
                            try:
                                ...
                                check_stack_status = cloudformation.describe_stacks(StackName=stack_id)
                                logpush('Checking existing cloudformation stack_id: ' +str(stack_id) + ' status: ' + str(check_stack_status['Stacks'][0]['StackStatus']))
                                    ...
                                    for resource in response['StackResources']:
                                        if resource['ResourceType'] == 'AWS::EC2::SpotFleet':
                                            ...
                                            if (spot_fleet_activity_status == 'fulfilled' or spot_fleet_activity_status == 'pending_fulfillment') and spot_fleet_request_state == 'active':
                                                logpush('Updating TargetCapacity for SpotFleet: ' + spotfleet + ' to: ' + str(new_target_capacity))
                                                resp=ec2.modify_spot_fleet_request(SpotFleetRequestId=spotfleet, TargetCapacity=new_target_capacity)
                                            else:
                                                # Revert stack_id and compute_node so the jobs can be reconsidered in the next dispatcher iteration
                                                logpush("Spot Fleet can't be modified at this time... reverting stack_id and select for job_ids:  " + str(jobs_to_revert))
                                        ...
                                        if resource['ResourceType'] == 'AWS::AutoScaling::AutoScalingGroup':
                                            asg = resource['PhysicalResourceId']
                                            response=autoscaling.describe_auto_scaling_groups(AutoScalingGroupNames=[asg], MaxRecords=100)
                                            new_target_capacity = int(response['AutoScalingGroups'][0]['DesiredCapacity']) + hash_cpu_ct
                                            logpush('Updating DesiredCapacity for ASG: ' + asg + ' to: ' + str(new_target_capacity))
                                            resp=autoscaling.update_auto_scaling_group(AutoScalingGroupName=asg, MinSize=new_target_capacity, 
                                                    MaxSize=new_target_capacity, DesiredCapacity=new_target_capacity)
                                            break
                                elif check_stack_status['Stacks'][0]['StackStatus'] == 'CREATE_IN_PROGRESS':
                                    # Revert stack_id and compute_node so the jobs can be reconsidered in the next dispatcher iteration
                                    logpush("CFN status is CREATE_IN_PROGRESS... reverting stack_id and select for job_ids:  " + str(jobs_to_revert))
                                    for job_id in jobs_to_revert:
                                        job_select_resource = hash_data[job_id]['get_job_resource_list']['select']
                                        new_job_select = job_select_resource.split(':compute_node')[0] + ':compute_node=tbd'
                                        run_command([system_cmds['qalter'], "-l", "stack_id=", "-l", "select=" + new_job_select, str(job_id)], "call")
                                ...
                                if e.response['Error']['Code'] == 'ValidationError':
                                    ...
                                    logpush('Unable to find a cfn stack for job_hash: ' + job_hash + ' ... Creating a new stack')
                                    ...
                                    job_parameter_values['job_name'] = job_data['get_job_name'] # May not be true for all jobs in the hash to have the same name
                                    job_parameter_values['job_owner'] = job_data['get_job_owner'] # May not be true for all jobs in the hash to have the same owner
                                    job_parameter_values['job_project'] = job_data['get_job_project'] # May not be true for all jobs in the hash to have the same project
                                    job_parameter_values['keep_forever'] = False
                                    job_parameter_values['terminate_when_idle'] = job_data['get_job_terminate_when_idle']
                                    job_parameter_values['instance_type'] = '+'.join(instance_types)
                                    job_parameter_values['ht_support'] = job_data['get_job_ht_support']
                                    job_parameter_values['weighted_capacity'] = '+'.join(str(item) for item in weighted_capacity)
