...
security_groups = j.Resource_List['security_groups'] if 'security_groups' in j.Resource_List else None
instance_profile = j.Resource_List['instance_profile'] if 'instance_profile' in j.Resource_List else None
        ...
        for k, v in doc.items():
            ...
            if job_queue in queues:
                try:
                    ...
                    e.reject("allowed_security_group_ids or allowed_instance_profiles is missing on " + queue_settings_file + " or are not valid Python lists")

                if security_groups is not None:
                    for sg_id in str(security_groups).split("+"):
                        if sg_id not in allowed_security_group_ids:
                            e.reject("Security group " + sg_id + " is not authorized for this queue. Please enable it on " + queue_settings_file + ". List of valid SG for this queue: " + str(allowed_security_group_ids))

                if instance_profile is not None:
                    if str(instance_profile) not in allowed_instance_profiles:
                        e.reject("IAM instance profile " + instance_profile + " is not authorized for this queue. Please enable it on " + queue_settings_file + ". List of instance profiles for this queue: " + str(allowed_instance_profiles))