...
def find_users_in_ldap_group(group_dn):
    if os.path.isdir("/apps/soca/%SOCA_CONFIGURATION/cluster_node_bootstrap/ad_automation"):
        ...
        # Active Directory
        with open(
            "/apps/soca/%SOCA_CONFIGURATION/cluster_node_bootstrap/ad_automation/join_domain_user.cache", "r"
        ) as f:
            ad_user = f.read()
        ...
        ldapsearch = (
            ...
            + '" -b "'
            + group_dn
            + '" | grep member  | awk \'{print $2}\' | cut -d, -f1 | tr -d "CN="'
        )
        pbs.logmsg(
            pbs.LOG_DEBUG,
            "queue_acl: generated ldapsearch command: " + ldapsearch.replace(ad_password, "<REDACTED_PASSWORD>"),
        )
    else:
        # OpenLdap
