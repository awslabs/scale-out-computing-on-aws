...
SOCA_FILESYSTEMS_SSM_TREE=$(aws_cli ssm get-parameters-by-path --path "/soca/soca-demoparis/configuration/FileSystems"  \
  --recursive \
  --with-decryption \
  --query "Parameters" \
  --output json)
...
FS_MAP=$(echo "$SOCA_FILESYSTEMS_SSM_TREE" | jq -r --arg prefix "/soca/soca-demoparis/configuration/FileSystems/" '
