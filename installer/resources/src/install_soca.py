...
try:
    ...
    print(
        " > You must have 'colored', 'boto3' and 'requests' installed. Run 'pip install boto3 colored requests' first"
    )
    sys.exit(1)
import argparse
import datetime
    ...
    try:
        ...
        for path, subdirs, files in os.walk(f"{dist_directory}"):
            ...
            for file in files:
                ...
                print(
                    f"{fg('green')}[+] Uploading {os.path.join(path, file)} to s3://{bucket}/{upload_location} {attr('reset')}"
                )
...
def accepted_aws_resources():
    ...
    try:
        ...
        print(
            f"{fg('yellow')}Unable to list SSH keys, you will need to enter it manually or give ec2:Describe* IAM permission. {err} {attr('reset')}"
        )
...
def default_ldap_user_password():
    ...
    if args.ldap_password_file is None:
        install_parameters["ldap_password"] = get_input(
            f"[Step 4/{total_install_phases}] {install_phases[4]}", args.ldap_password, None, str, hide=True
        )
        while (
            not re.match(password_regex, install_parameters["ldap_password"])
            ...
            print(
                f"{fg('red')} LDAP password must have 1 uppercase, 1 lowercase, 1 digit and be 8 chars min.\n LDAP password cannot contain your username, white space, or any of the following special characters {''.join(restricted_chars)}{attr('reset')}"
            )
            install_parameters["ldap_password"] = get_input(
                f"[Step 4/{total_install_phases}] {install_phases[4]}", None, None, str, hide=True
            )
        ...
        if not args.ldap_password:
            ...
            if install_parameters["ldap_password"] != ldap_password_verify:
                print(
                    f"{fg('red')} You entered two different passwords. Please try again and make sure password and password (verification) are the same.{attr('reset')}"
                )
                return False
            else:
                return True
        ...
        try:
            with open(args.ldap_password_file) as f:
                ...
                if not re.match(password_regex, install_parameters["ldap_password"]):
                    print(
                        f"{fg('red')} LDAP password must have 1 uppercase, 1 lowercase, 1 digit and be 8 char min{attr('reset')}"
                    )
                    sys.exit(1)
        except FileNotFoundError:
...
def get_install_parameters():
    ...
    while len(install_parameters["cluster_name"]) < 3 or len(install_parameters["cluster_name"]) > 11:
        print(
            f"{fg('red')}SOCA cluster name must greater than 3 chars and shorter than 11 characters (soca- is automatically added as a prefix) {attr('reset')}"
        )
        install_parameters["cluster_name"] = get_input(
    ...
    install_parameters["bucket"] = get_input(
        f"[Step 2/{total_install_phases}] {install_phases[2]}", args.bucket, None, str
    )
    ...
    install_parameters["ldap_user"] = get_input(
        f"[Step 3/{total_install_phases}] {install_phases[3]}", args.ldap_user, None, str
    )
    ...
    if install_props.Config.directoryservice.provider == "activedirectory":
        while install_parameters["ldap_user"].lower() == "admin":
            print(
                f"{fg('yellow')} To prevent conflict with Directory Service, the first SOCA user cannot be named admin. Please pick a different name.{attr('reset')}"
            )
            install_parameters["ldap_user"] = get_input(
                f"[Step 3/{total_install_phases}] {install_phases[3]}", None, None, str
            )
    ...
    install_parameters["base_os"] = get_input(
        f"[Step 5/{total_install_phases}] {install_phases[5]}", args.base_os, ["amazonlinux2", "centos7", "rhel7"], str
    )
    install_parameters["ssh_keypair"] = get_input(
        f"[Step 6/{total_install_phases}] {install_phases[6]}",
        args.ssh_keypair,
        accepted_aws_values["accepted_keypairs"],
        str,
    )
    ...
    if args.prefix_list_id:
        try:
            found_prefix_list_id = ec2.describe_managed_prefix_lists(PrefixListIds=[args.prefix_list_id])[
                "PrefixLists"
            ][0]["PrefixListId"]
            ...
            print(
                f"{fg('red')}Error. {args.prefix_list_id} not found. Check that it exists and starts with pl-.\nException:\n{e} {attr('reset')}"
            )
            sys.exit(1)
    ...
    if not args.vpc_cidr:
        ...
        if choice_vpc == "new":
            install_parameters["vpc_cidr"] = get_input(
                "What CIDR do you want to use your your VPC? We recommend 10.0.0.0/16", args.vpc_cidr, None, str
            )
            while not re.match(cidr_regex, install_parameters["vpc_cidr"]):
                print(
                    f"{fg('red')} Invalid CIDR {install_parameters['vpc_cidr']}. Format must be x.x.x.x/x (eg: 10.0.0.0/16){attr('reset')}"
                )
                install_parameters["vpc_cidr"] = get_input(
                    "What CIDR do you want to use your your VPC? We recommend 10.0.0.0/16", None, None, str
                )
        else:
            ...
            private_subnets = FindExistingResource(
                install_parameters["region"], install_parameters["client_ip"]
            ).get_subnets(install_parameters["vpc_id"], "private", public_subnets["message"])
    ...
    if install_parameters["vpc_id"]:
        choice_security_groups = get_input(
            f"[Step 8/{total_install_phases}] {install_phases[8]}", None, ["new", "existing"], str
        )
        if choice_security_groups == "existing":
            ...
            compute_node_sg = FindExistingResource(
                install_parameters["region"], install_parameters["client_ip"]
            ).get_security_groups(install_parameters["vpc_id"], "compute nodes", install_parameters["scheduler_sg"])
            ...
            if install_props.Config.network.vpc_interface_endpoints:
                vpc_endpoint_sg = FindExistingResource(
                    install_parameters["region"], install_parameters["client_ip"]
                ).get_security_groups(install_parameters["vpc_id"], "vpc endpoints", install_parameters["scheduler_sg"])
    ...
    if install_parameters["vpc_id"]:
        choice_filesystem = get_input(
            f"[Step 9/{total_install_phases}] {install_phases[9]}", None, ["new", "existing"], str
        )
        if choice_filesystem == "existing":
            # List FS
            fs_apps = FindExistingResource(install_parameters["region"], install_parameters["client_ip"]).get_fs(
                "/apps", install_parameters["vpc_id"]
            )
            ...
            fs_data = FindExistingResource(install_parameters["region"], install_parameters["client_ip"]).get_fs(
                "/data", install_parameters["vpc_id"], selected_fs=install_parameters["fs_apps"]
            )
    ...
    if install_parameters["fs_apps"] or install_parameters["scheduler_sg"]:
        FindExistingResource(install_parameters["region"], install_parameters["client_ip"]).validate_sg_rules(
            install_parameters, check_fs=True if install_parameters["fs_apps"] else False
        )
    ...
    if install_props.Config.directoryservice.provider == "activedirectory":
        if install_parameters["vpc_id"]:
            choice_mad = get_input(
                f"[Step 10/{total_install_phases}] {install_phases[10]}", None, ["new", "existing"], str
            )
            if choice_mad == "existing":
                ...
                if directory_service["success"] is True:
                    install_parameters["directory_service_ds_user"] = get_input(
                        f"Username of a domain user with  admin permissions?", None, None, str
                    )
                    install_parameters["directory_service_ds_user_password"] = get_input(
                        f"Password of the domain user with admin permissions", None, None, str
                    )
    ...
    if install_parameters["vpc_id"]:
        choice_iam_roles = get_input(
            f"[Step 12/{total_install_phases}] {install_phases[12]}", None, ["new", "existing"], str
        )
        if choice_iam_roles == "existing":
            ...
            compute_node_role = FindExistingResource(
                install_parameters["region"], install_parameters["client_ip"]
            ).get_iam_roles("compute nodes", selected_roles=[install_parameters["scheduler_role_name"]])
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--ldap-user", "-lu", type=str, help="Username of your first ldap user. This user has admin privileges"
    )
    ...
    parser.add_argument(
        "--vpc-cidr", "-cidr", type=str, help="What CIDR do you want to use your your VPC (eg: 10.0.0/16)"
    )
    parser.add_argument("--client-ip", "-ip", type=str, help="Client IP authorized to access SOCA on port 22/443")
    parser.add_argument(
        "--prefix-list-id", "-pl", type=str, help="Prefix list ID with IPs authorized to access SOCA on port 22/443"
    )
    parser.add_argument(
        "--name",
        "-n",
        type=str,
        help="Friendly name for your SOCA cluster. Must be unique. SOCA will be added as prefix",
    )
    parser.add_argument(
        "--base-os",
        "-os",
        choices=["amazonlinux2", "centos7", "rhel7"],
        type=str,
    ...
    parser.add_argument(
        "--cdk-cmd",
        type=str,
        choices=["deploy", "create", "update", "ls", "list", "synth", "synthesize", "destroy", "bootstrap"],
        default="deploy",
    )
    parser.add_argument(
        "--skip-config-message", action="store_const", const=True, default=False, help="Skip default_config message"
    )
    ...
    install_props = json.loads(
        json.dumps(get_install_properties(args.config)), object_hook=lambda d: SimpleNamespace(**d)
    )
    ...
    install_parameters["region"] = get_input(
        "What AWS region do you want to install SOCA?", args.region, accepted_regions, str
    )
    ...
    cloudformation = session.client(
        "cloudformation", region_name=install_parameters["region"], config=boto_extra_config
    )
    ...
    try:
        ...
        print(
            f"{fg('red')}Unable to determine if you have a ServiceLinked created on your account for ElasticSearch. Verify your IAM permissions: {err} {attr('reset')}"
        )
        sys.exit(1)
    ...
    if not args.client_ip:
        ...
        print(
            f"{fg('yellow')}We determined your IP is {install_parameters['client_ip']}. You can change it later if you are running behind a proxy{attr('reset')}"
        )
        if install_parameters["client_ip"] is False:
            install_parameters["client_ip"] = get_input(
                "Client IP authorized to access SOCA on port 443/22", args.client_ip, None, str
            )
    else:
    ...
    if install_parameters["client_ip"].endswith("/32"):
        ...
        if "/" not in install_parameters["client_ip"]:
            print(
                f"{fg('yellow')}No subnet defined for your IP. Adding /32 at the end of {install_parameters['client_ip']}{attr('reset')}"
            )
    ...
    try:
        ...
        if len(check_if_name_exist["Stacks"]) != 0:
            if args.cdk_cmd == "create":
                print(
                    f"{fg('red')}Error. {install_parameters['cluster_id']} already exists in CloudFormation. Please pick a different name (soca- is automatically added as a prefix).{attr('reset')}"
                )
                sys.exit(1)
            elif args.cdk_cmd == "deploy":
                print(
                    f"{fg('red')}Error. {install_parameters['cluster_id']} already exists in CloudFormation. Use --cdk-cmd update if you want to update it.{attr('reset')}"
                )
                sys.exit(1)
    except ClientError as e:
        if e.response["Error"]["Code"] == "ValidationError":
            if args.cdk_cmd == "update":
                print(
                    f"{fg('red')}Error. {install_parameters['cluster_id']} does not exist in CloudFormation so can't be updated. Use --cdk-cmd deploy if you want to create it.{attr('reset')}"
                )
                sys.exit(1)
            else:
            ...
            print(
                f"{fg('red')}Error checking if {install_parameters['cluster_id']} already exists in CloudFormation due to {e}.{attr('reset')}"
            )
            sys.exit(1)
...
============================="""
    ...
    if int(bootstrap_environment) != 0:
        print(
            f"{fg('red')} Error! Unable to bootstrap environment. Please run {cmd_bootstrap} and fix any errors{attr('reset')}"
        )
                ...
                for output in check_cfn["Stacks"][0]["Outputs"]:
                    if output["OutputKey"] == "WebUserInterface":
                        ...
                        try:
                            ...
                            print(
                                f"{fg('yellow')}Unable to connect to the SOCA endpoint URL. Maybe your IP {install_parameters['client_ip']} is not valid/has changed (maybe you are behind a proxy?). If that's the case please go to AWS console and authorize your real IP on the Scheduler Security Group{attr('reset')}"
                            )
                            sys.exit(1)
                        soca_check_loop = 0
                        if install_parameters["vpc_id"]:
                        ...
                        while (
                            get(output["OutputValue"], verify=False, timeout=15).status_code != 200
                            ...
                            if soca_check_loop == max_check_loop:
                                print(
                                    f"{fg('yellow')}Could not determine if SOCA is ready after 20 minutes. Connect to the system via SSM and check the logs. {attr('reset')}"
                                )
                                sys.exit(1)
