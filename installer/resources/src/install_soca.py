...
def get_install_properties(config_file_path):
    ...
    try:
        ...
        print(
            f"{config_file_path} not found. Make sure the file exist and the path is correct."
        )
        sys.exit(1)
...
def build_lambda_dependency(install_directory):
    ...
    for dir in os.scandir(lambda_functions_folders):
...
def accepted_aws_resources():
    ...
    try:
        accepted_values["accepted_keypairs"] = [
            key["KeyName"] for key in ec2.describe_key_pairs()["KeyPairs"]
        ]
        if len(accepted_values) == 0:
            print(
                f"{fg('red')} No SSH keys found on this region. Please create one first{attr('reset')}"
            )
            sys.exit(1)
...
def check_bucket_permission(bucket):
    ...
    try:
        ...
        print(
            f"{fg('red')}The bucket {bucket} does not exist or you have no access: {e}{attr('reset')}"
        )
        return False
...
def default_ldap_user_password():
    ...
    if args.ldap_password_file is None:
        ...
        print(
            f"{fg('yellow')}Retrieving password from {args.ldap_password_file}{attr('reset')}"
        )
        try:
            ...
            print(
                f"{fg('red')}Unable to found {args.ldap_password_file}. Please specify absolute path {attr('reset')}"
            )
            sys.exit(1)
...
def get_install_parameters():
    ...
    while (
        ...
        print(
            f"{fg('red')}LDAP user must be 5 characters mins and can only contains alphanumeric.{attr('reset')}"
        )
        install_parameters["ldap_user"] = get_input(
    ...
    install_parameters["base_os"] = get_input(
        f"[Step 5/{total_install_phases}] {install_phases[5]}",
        args.base_os,
        [
            "amazonlinux2",
            "centos7",
        ...
        if choice_vpc == "new":
            ...
            if install_props.Config.entry_points_subnets.lower() == "public":
                ...
                if public_subnets["success"] is True:
                    ...
                    print(
                        f"{fg('red')}Error: {public_subnets['message']} {attr('reset')}"
                    )
                    sys.exit(1)
            else:
        ...
        if choice_security_groups == "existing":
            ...
            if install_props.Config.network.vpc_interface_endpoints:
                ...
                if vpc_endpoint_sg["success"] is True:
                    ...
                    print(
                        f"{fg('red')}Error: {vpc_endpoint_sg['message']} {attr('reset')}"
                    )
                    sys.exit(1)
            else:
    ...
    if install_parameters["vpc_id"]:
        ...
        if choice_filesystem == "existing":
            ...
            fs_apps = FindExistingResource(
                install_parameters["region"], install_parameters["client_ip"]
            ).get_fs("/apps", install_parameters["vpc_id"])
            ...
            fs_data = FindExistingResource(
                ...
                "/data",
                install_parameters["vpc_id"],
                selected_fs=install_parameters["fs_apps"],
            )
            if fs_data["success"] is True:
                ...
                if install_parameters["fs_data"] == install_parameters["fs_apps"]:
                    print(
                        f"{fg('red')}Error: EFS/FSx for Lustre /apps and /data must be different {attr('reset')}"
                    )
                    sys.exit(1)
            else:
        ...
        if install_parameters["vpc_id"]:
            ...
            if choice_mad == "existing":
                ...
                if directory_service["success"] is True:
                    ...
                    print(
                        f"{fg('red')}Error: {directory_service['message']} {attr('reset')}"
                    )
                    sys.exit(1)
    ...
    if install_parameters["vpc_id"]:
        ...
        if choice_es == "existing":
            ...
            if elasticsearch_cluster["success"] is True:
                ...
                print(
                    f"{fg('red')}Error: {elasticsearch_cluster['message']} {attr('reset')}"
                )
                sys.exit(1)
    else:
    ...
    if install_parameters["vpc_id"]:
        ...
        if choice_iam_roles == "existing":
            ...
            if compute_node_role["success"] is True:
                ...
                print(
                    f"{fg('red')}Error: {compute_node_role['message']} {attr('reset')}"
                )
                sys.exit(1)
...
if __name__ == "__main__":
    ...
    install_properties = get_install_properties(args.config)
    if args.override:
        overrides = [item for sublist in args.override for item in sublist]
    ...
    if args.profile:
        try:
            ...
            print(
                f"{fg('red')} Profile {args.profile} not found. Check ~/.aws/credentials file{attr('reset')}"
            )
            sys.exit(1)
    else:
    ...
    install_parameters["region"] = get_input(
        "What AWS region do you want to install SOCA?",
        args.region,
        accepted_regions,
        str,
    )
    ...
    try:
        ...
        print(
            f"{fg('red')} Unable to retrieve the Account ID due to {err}{attr('reset')}"
        )
        sys.exit(1)
                ...
                for output in check_cfn["Stacks"][0]["Outputs"]:
                    if output["OutputKey"] == "WebUserInterface":
                        ...
                        try:
                            ...
                            print(
                                f"{fg('yellow')}WARNING - Unable to connect to the SOCA endpoint URL. Error: {e} {attr('reset')}"
                            )
                        ...
                        while (
                            ...
                            if soca_check_loop >= max_check_loop:
                                print(
                                    f"{fg('yellow')}Could not determine if SOCA is ready after {max_check_loop*2} minutes. Connect to the system via SSM and check the logs. {attr('reset')}"
                                )
                                sys.exit(1)

                        print(
                            f"{fg('green')}SOCA is ready! Login via  {output['OutputValue']}{attr('reset')}"
                        )
