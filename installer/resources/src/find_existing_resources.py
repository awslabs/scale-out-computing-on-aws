...
class FindExistingResource:
    ...
    def get_subnets(self, vpc_id, environment, selected_subnets=[]):
        try:
            ...
            while token is True:
                if not next_token:
                    all_subnets = self.ec2.describe_subnets(
                        ...
                        Filters=[{"Name": "vpc-id", "Values": [vpc_id]}, {"Name": "ipv6-native", "Values": ["false"]}],
                    )
                else:
                    all_subnets = self.ec2.describe_subnets(
                        Filters=[{"Name": "vpc-id", "Values": [vpc_id]}, {"Name": "ipv6-native", "Values": ["false"]}],
            ...
            selected_subnets_count = get_input(
                f"How many of these subnets do you want to use?", None, list(range(1, count)), int
            )
        ...
        try:
            ...
            while marker is True:
                ...
                for filesystem in all_efs["FileSystems"]:
                    verified_vpc = False
                    for mount_target in self.efs.describe_mount_targets(FileSystemId=filesystem["FileSystemId"])[
                        "MountTargets"
                    ]:
    ...
    def get_security_groups(self, vpc_id, environment, scheduler_sg=[]):
        try:
            ...
            while token is True:
                if not next_token:
                    all_sgs = self.ec2.describe_security_groups(
                        MaxResults=max_results, Filters=[{"Name": "vpc-id", "Values": [vpc_id]}]
                    )
                else:
                    all_sgs = self.ec2.describe_security_groups(
                        MaxResults=max_results, NextToken=next_token, Filters=[{"Name": "vpc-id", "Values": [vpc_id]}]
                    )
                try:
            ...
            choice = get_input(
                f"What security group for you want to use for {environment.upper()}", None, allowed_choices, int
            )
    ...
    def get_iam_roles(self, environment, selected_roles=[]):
        try:
            ...
            choice = get_input(
                f"What IAM Role for you want to use for {environment.upper()}", None, allowed_choices, int
            )
        ...
        try:
            ...
            for id in efs_ids:
                for mount in self.efs.describe_mount_targets(FileSystemId=id.split(".")[0])["MountTargets"]:
                    for sg in self.efs.describe_mount_target_security_groups(MountTargetId=mount["MountTargetId"])[
                        "SecurityGroups"
                    ]:
                        if sg not in sgs:
                            sgs.append(sg)
            ...
            for id in fsx_ids:
                for network_interface in self.fsx.describe_file_systems(FileSystemIds=[id])["FileSystems"][0][
                    "NetworkInterfaceIds"
                ]:
    ...
    def validate_sg_rules(self, cfn_params, check_fs=True):
        try:
            ...
            if check_fs is True:
                if (
                    cfn_params["scheduler_sg"] in fs_sg["message"][cfn_params["fs_apps"]]
                    and cfn_params["compute_node_sg"] in fs_sg["message"][cfn_params["fs_apps"]]
                ):
                    errors["FS_APP_SG"]["status"] = True

                if (
                    cfn_params["scheduler_sg"] in fs_sg["message"][cfn_params["fs_data"]]
