    ...
    if specified_value:
        ...
        while isinstance(response, expected_type) is False:
            if sys.version_info[0] >= 3:
                if expected_answers:
                    question = input(f"{fg(color)} >> {prompt} {expected_answers}{attr('reset')}: ")
                else:
                    if hide is True:
                        question = getpass.getpass(prompt=f"{fg(color)} >> {prompt}{attr('reset')}: ")
                    else:
                        question = input(f"{fg(color)} >> {prompt}{attr('reset')}: ")
            else:
                # Python 2
                if expected_answers:
                    question = raw_input(f"{fg(color)} >> {prompt} {expected_answers}{attr('reset')}: ")
                else:
