...
class SOCAInstall(cdk.Stack):
    def __init__(self, scope: cdk.Construct, construct_id: str, **kwargs) -> None:
        ...
        self.soca_resources = {
            ...
            "ami_id": user_specified_variables.custom_ami
            if user_specified_variables.custom_ami
            else install_props.RegionMap.__dict__[user_specified_variables.region].__dict__[
                user_specified_variables.base_os
            ],
    ...
    def generic_resources(self):
        ...
        self.tag_ec2_resource_lambda.add_to_role_policy(
            statement=iam.PolicyStatement(effect=iam.Effect.ALLOW, actions=["ec2:CreateTags"], resources=["*"])
        )
            ...
            vpc_params = {
                ...
                "subnet_configuration": [
                    ec2.SubnetConfiguration(
                        cidr_mask=public_subnet_mask, name="Public", subnet_type=ec2.SubnetType.PUBLIC
                    ),
                    ec2.SubnetConfiguration(
                        cidr_mask=private_subnet_mask, name="Private", subnet_type=ec2.SubnetType.PRIVATE
                    ),
                ],
            }
    ...
    def security_groups(self):
        ...
        if not user_specified_variables.compute_node_sg:
            ...
            core.Tags.of(self.soca_resources["compute_node_sg"]).add(
                "Name", f"{user_specified_variables.cluster_id}-ComputeNodeSG"
            )
        else:
            self.soca_resources["compute_node_sg"] = ec2.SecurityGroup.from_security_group_id(
                self, "ComputeNodeSG", security_group_id=user_specified_variables.compute_node_sg
            )
        ...
        if not user_specified_variables.scheduler_sg:
            ...
            # We do not use `security_group_name` as it's not recommended in case you plan to do UPDATE_TEMPLATE in the future. We assign a Name tag instead
            core.Tags.of(self.soca_resources["scheduler_sg"]).add(
                "Name", f"{user_specified_variables.cluster_id}-SchedulerSG"
            )
        else:
            self.soca_resources["scheduler_sg"] = ec2.SecurityGroup.from_security_group_id(
                self, "SchedulerSG", security_group_id=user_specified_variables.scheduler_sg
            )
        ...
        if not user_specified_variables.vpc_endpoint_sg:
            ...
            core.Tags.of(self.soca_resources["vpc_endpoint_sg"]).add(
                "Name", f"{user_specified_variables.cluster_id}-VpcEndpointSG"
            )
        else:
            self.soca_resources["vpc_endpoint_sg"] = ec2.SecurityGroup.from_security_group_id(
                self,
                "VpcEndpointSG",
        ...
        self.soca_resources["compute_node_sg"].add_egress_rule(
            ec2.Peer.ipv4("0.0.0.0/0"), ec2.Port.tcp_range(0, 65535), description="Allow all egress"
        )
    ...
    def create_vpc_endpoints(self):
                ...
                for vpc_endpoint in page["VpcEndpoints"]:
                    ...
                    if vpc_endpoint["VpcEndpointType"] == "Gateway":
                        self.vpc_gateway_endpoints[
                            ...
                            self, resource_name, gateway_vpc_endpoint_id=vpc_endpoint["VpcEndpointId"]
                        )
        ...
        for short_service_name in install_props.Config.network.vpc_gateway_endpoints:
            ...
            self.vpc_gateway_endpoints[short_service_name] = self.soca_resources["vpc"].add_gateway_endpoint(
                resource_name, service=endpoint_service
            )
            core.CustomResource(
                self,
    ...
    def iam_roles(self):
        ...
        if use_existing_roles is False:
            ...
            self.soca_resources["scheduler_role"] = iam.Role(
                ...
                assumed_by=iam.CompositePrincipal(
                    iam.ServicePrincipal(principals_suffix["ssm"]), iam.ServicePrincipal(principals_suffix["ec2"])
                ),
            )
            ...
            self.soca_resources["compute_node_instance_profile"] = iam.CfnInstanceProfile(
                self, "ComputeNodeInstanceProfile", roles=[self.soca_resources["compute_node_role"].role_name]
            )
        else:
            # Reference existing Scheduler/ComputeNode/SpotFleet roles
            self.soca_resources["scheduler_role"] = iam.Role.from_role_arn(
                self, "SchedulerRole", role_arn=user_specified_variables.scheduler_role_arn
            )
            self.soca_resources["compute_node_role"] = iam.Role.from_role_arn(
                self, "ComputeNodeRole", role_arn=user_specified_variables.compute_node_role_arn
            )
            self.soca_resources["spot_fleet_role"] = iam.Role.from_role_arn(
                self, "SpotFleetRole", role_arn=user_specified_variables.spotfleet_role_arn
            )
            self.soca_resources["compute_node_instance_profile"] = iam.CfnInstanceProfile(
                self, "ComputeNodeInstanceProfile", roles=[user_specified_variables.compute_node_role_name]
            )
        ...
        if install_props.Config.directoryservice.provider == "activedirectory":
            self.soca_resources["reset_ds_password_lambda_role"] = iam.Role(
                self, "ResetDsLambdaLambdaRole", assumed_by=iam.ServicePrincipal(principals_suffix["lambda"])
            )
    ...
    def directoryservice(self):
        ...
        if not user_specified_variables.directory_service_id:
            ...
            ] = f"{random.choice(string.ascii_lowercase)}{random.choice(string.digits)}{random.choice(string.ascii_uppercase)}{''.join(random.choice(string.ascii_lowercase + string.digits + string.ascii_uppercase) for i in range(20))}"
            ...
            resolver_rule = route53resolver.CfnResolverRule(
                ...
                target_ips=[
                    route53resolver.CfnResolverRule.TargetAddressProperty(
                        ip=core.Fn.select(0, self.soca_resources["directory_service"].attr_dns_ip_addresses), port="53"
                    ),
                    route53resolver.CfnResolverRule.TargetAddressProperty(
                        ip=core.Fn.select(1, self.soca_resources["directory_service"].attr_dns_ip_addresses), port="53"
                    ),
                ],
            )
    ...
    def storage(self):
        ...
        if install_props.Config.storage.data.provider == "efs" and not user_specified_variables.fs_data:
            for i in range(
                len(self.soca_resources["vpc"].select_subnets(subnet_type=ec2.SubnetType.PRIVATE).subnet_ids)
            ):
                efs.CfnMountTarget(
                    self,
                    ...
                    subnet_id=self.soca_resources["vpc"]
        ...
        if install_props.Config.storage.apps.provider == "efs" and not user_specified_variables.fs_apps:
            ...
            efs_apps_cw_alarm_low = cloudwatch.Alarm(
                ...
                metric=cloudwatch.Metric(
                    ...
                    namespace="AWS/EFS",
                    dimensions=dict(FileSystemId=self.soca_resources["fs_apps"].ref),
                ),
        ...
        if install_props.Config.storage.data.provider == "fsx_lustre" and not user_specified_variables.fs_data:
            ...
            self.soca_resources["fs_data"] = fsx.CfnFileSystem(
                ...
                subnet_ids=[
                    self.soca_resources["vpc"].select_subnets(subnet_type=ec2.SubnetType.PRIVATE).subnets[0].subnet_id
                ],
    ...
    def scheduler(self):
        ...
        if not user_specified_variables.vpc_id:
            ...
            launch_subnet = ec2.Subnet.from_subnet_attributes(
                self, "SubnetToUse", availability_zone=subnet_info[1], subnet_id=subnet_info[0]
            )
        ...
        core.Tags.of(self.soca_resources["scheduler_instance"]).add(
    ...
    def secretsmanager(self):
        ...
        self.soca_resources["scheduler_role"].attach_inline_policy(
            iam.Policy(self, "AttachSecretManagerPolicyToScheduler", statements=[secret_manager_statement])
        )
        self.soca_resources["compute_node_role"].attach_inline_policy(
            iam.Policy(self, "AttachSecretManagerPolicyToComputeNode", statements=[secret_manager_statement])
        )
    ...
    def analytics(self):
        ...
        if not user_specified_variables.es_endpoint:
            ...
            self.soca_resources["es_domain"] = elasticsearch.Domain(
                self,
                "ElasticSearch",
                ...
                ebs=elasticsearch.EbsOptions(
                    volume_size=install_props.Config.elasticsearch.ebs_volume_size,
                ...
                if install_props.Config.elasticsearch.deletion_policy.upper() == "RETAIN"
            ...
            self.soca_resources["es_custom_resource"] = core.CustomResource(
                self,
                "ESCustomResource",
            ...
            self.soca_resources["es_custom_resource"].node.add_dependency(
    ...
    def viewer(self):
        ...
        cert_custom_resource = core.CustomResource(
            self,
        ...
        soca_webui_target_group = elbv2.CfnTargetGroup(
            ...
            targets=[
                elbv2.CfnTargetGroup.TargetDescriptionProperty(id=self.soca_resources["scheduler_instance"].instance_id)
            ],
        ...
        https_listener = elbv2.CfnListener(
            ...
            certificates=[
                elbv2.CfnListener.CertificateProperty(
                    certificate_arn=cert_custom_resource.get_att_string("ACMCertificateArn")
                )
            ],
            default_actions=[
                elbv2.CfnListener.ActionProperty(type="forward", target_group_arn=soca_webui_target_group.ref)
            ],
        )
            ...
            for i in range(0, install_props.Config.elasticsearch.data_nodes * 3):
                es_targets.append(
                    elbv2.CfnTargetGroup.TargetDescriptionProperty(
                        id=core.Fn.select(
                            i,
                            core.Fn.split(",", self.soca_resources["es_custom_resource"].get_att_string("IpAddresses")),
                        )
                    )
                )
        ...
        if not user_specified_variables.es_endpoint:
            core.CfnOutput(
                self,
                "AnalyticsDashboard",
                value=f"https://{self.soca_resources['alb'].load_balancer_dns_name}/_plugin/kibana/",
            )
        else:
            core.CfnOutput(
                self, "AnalyticsDashboard", value=f"https://{user_specified_variables.es_endpoint}/_plugin/kibana/"
            )
...
if __name__ == "__main__":
        ...
        json.dumps(
            {
                ...
                else ast.literal_eval(base64.b64decode(app.node.try_get_context("public_subnets")).decode("utf-8")),
                "private_subnets": app.node.try_get_context("private_subnets")
                if app.node.try_get_context("private_subnets") is None
                else ast.literal_eval(base64.b64decode(app.node.try_get_context("private_subnets")).decode("utf-8")),
    ...
    endpoints_suffix = {
        "fsx_lustre": f"fsx.{core.Aws.REGION}.{core.Aws.URL_SUFFIX}",
        "efs": f"efs.{core.Aws.REGION}.{core.Aws.URL_SUFFIX}",
    }

    principals_suffix = {
        "backup": f"backup.{core.Aws.URL_SUFFIX}",
