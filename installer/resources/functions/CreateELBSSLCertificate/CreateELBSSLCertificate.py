    ...
    if event["RequestType"] == "Delete":
        ...
        if "ACMCertificateArn" in output.keys():
            ...
            subprocess.check_output(
                "openssl req -new -x509 -sha1 -nodes -days 3650  -key /tmp/server.key -subj '/C=US/ST=California/L=Sunnyvale/O="
                + ClusterId
                + "/CN=SOCA.DEFAULT.CREATE.YOUR.OWN.CERT' > /tmp/server.crt",
                shell=True,
            )  # nosec
            ...
            try:
                ...
                client_acm.add_tags_to_certificate(
                    CertificateArn=response["CertificateArn"], Tags=[{"Key": "Name", "Value": "Soca_ALB_Cerficate"}]
                )
